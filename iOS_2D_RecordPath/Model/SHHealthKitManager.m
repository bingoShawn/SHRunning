//
//  SHHealthKitManager.m
//  iOS_2D_RecordPath
//
//  Created by Shawn on 2018/12/27.
...
@implementation SHHealthKitManagerModel

@end
@implementation SHHealthKitManager
#pragma mark - **************** 单例创建
+ (id)shareInstance {
    static id manager ;
...
+ (NSArray <SHHealthKitManagerModel *>*)predicateForSamplesHour {
    ...
    for (int i = 0; i < count; i ++) {
        SHHealthKitManagerModel *model = [[SHHealthKitManagerModel alloc] init];
...
#pragma mark - 步数
- (void)getStepFromHealthKitComplate:(void(^)(NSArray <SHHealthKitManagerModel *> *stepArray))handler {
    HKQuantityType *stepType = [HKObjectType quantityTypeForIdentifier:HKQuantityTypeIdentifierStepCount];
    NSArray *arr = [SHHealthKitManager predicateForSamplesHour];
    __block NSMutableArray *arrM = [NSMutableArray array];
    for (int i = 0; i < arr.count; i ++) {
        SHHealthKitManagerModel *model = [arr objectAtIndex:i];
...
- (void)fetchSampleQuantitySumOfSamplesTodayForType:(HKSampleType *)sampleType unit:(HKUnit *)unit isLatest:(BOOL)isLatest completion:(void (^)(double,NSError *))completionHandler {
    NSPredicate *predicate = [SHHealthKitManager predicateForSamplesToday:isLatest];
...
- (void)fetchQuantitySumOfSamplesTodayForType:(HKQuantityType *)quantityType unit:(HKUnit *)unit completion:(void (^)(double, NSError *))completionHandler {
    NSPredicate *predicate = [SHHealthKitManager predicateForSamplesToday:NO];
